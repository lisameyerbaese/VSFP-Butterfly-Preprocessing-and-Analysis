...
%   1) take the global signal (i.e. GS volt or GS Hemo) and obtains correlation map for volt and hemo
%      (low pass <5Hz) signals
%
% Inputs: 
...
function VSFP_CorrMap_VoltHemo_globalSignal(varargin)
    ...
    FolderName = [startFile, '\VSFP ButterFly\Data\VSFP_CorrMap_VoltHemo\'];   % Your destination folder
    ...
    for m = 1: length(all_mice) 
        ...
        for k = 1:length(all_trials)
            ...
            % Correlation between gs volt and volt
            corr_map_temp = fastCorr2(data_temp_2d',gsVolt');
            ...
            % Correlation between gs hemo and hemo
            corr_map_temp = fastCorr2(data_temp_2d',gsHemo');
            corr_map = reshape(corr_map_temp,[100,100]);
            ...
            hold off
            alpha(a,.2)
            title('Max Corr Coef Hemo and GS Hemo')
            ...
            hold off
            alpha(a,.2)
            title('Max Corr Coef Volt and GS Volt')
            axis off;
            avgTrial_Volt(:,:,k) = masked_corr_map_reg_volt;
            
            fname = [mouse,'_0', num2str(date) ,'_',num2str(trial),'_CorrMap_Hemo.fig'];
            saveFig(f1, fname, FolderName);
            fname = [mouse,'_0', num2str(date) ,'_',num2str(trial),'_CorrMap_Volt.fig'];
