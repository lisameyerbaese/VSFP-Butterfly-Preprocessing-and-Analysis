function outData = pupil_processing(thr,play_video,save_file);
%%%
%
%   Processing Pipeline for Pupil Detection
%
%   Usage:
%       outData = pupil_processing([],0,0);
%     where,
%       thr, black/white threshhold (can be left empty "[]" and set later)
%       play_video, will play_video of pupil tracking if = 1
%       save_file, will save outData as .mat file if = 1
%
%   requires (embedded at end file):
%       maxDiameter.m
%       thresh_tool_rm.m
%   
%   Editied from pupil_processing_RM3.m Copyright MIT (c) 2017 Arthur Morrissette
%
%   Copyright MIT (c) 2021 Arthur Morrissette and Lisa Meyer-Baese
%%%
...
if lastPath == 0 % First time calling 'uigetfile', use the pwd
    [filename, pathname] = uigetfile('*.avi', 'Select pupil video file');
else % All subsequent calls, use the path to the last selected file
    [filename, pathname] = uigetfile([lastPath '/*.avi'], 'Select pupil video file');
end
...
if exist([pathname filename(1:end-4) '.mat'], 'file')
    user_in = input('File Exists - "yes" to continue: ');
    if user_in ~= 1
...
%% Read and display average of first 10 frames (to use for blink detection)
first_frames = zeros(movieObj.Height,movieObj.Width,5);

for i = 1:5
    a = read(movieObj,i);
    first_frames(:,:,i) = rgb2gray(a);
end

mean_frame = mean(first_frames,3);

% And display mean frame to make sure pupil video - click or press enter to proceed
h1 = figure;
imagesc(mean_frame), caxis([0 200]), colormap('gray')
disp('-     Confirm pupil in first frames: Click or ENTER to continue')
press = waitforbuttonpress;
if press == 1 || press == 0 % close figure and continue
    close(h1)
end

%% Crop window to extract pupil from larger FOV (optional)
disp('-     Select region to crop or exit figure')
h1 = figure;
imagesc(mean_frame), caxis([0 200]), colormap('gray')
try [coords] = getrect;
catch coords = 0;
...
if coords ~= 0
    x = floor(coords(1));
    y = floor(coords(2));
    width = floor(coords(3));
    height = floor(coords(4));
else
    x = 1;
    y = 1;
...
close(h1);
mean_frame = mean_frame(y:y+height,x:x+width);

%% Now adjust threshhold for pupil detection
if isempty(thr)
    default_thr = 0.75.*mean(mean(mean_frame));
    [thr,~] = thresh_tool_rm(mean_frame,'gray',default_thr);
else
    % do nothing - threshhold already set
end

%% Now do the pupil detection for the entire video

%numFrames = single(movieObj.FrameRate*movieObj.Duration);
numFrames = single(movieObj.NumFrames);

% Setup structure to save data
outData = struct('ver','1.0');
outData.date = date;

blink = zeros(1,numFrames);
pupil = zeros(1,numFrames);
outPts = zeros(2,2,numFrames);

disp(['-     ' num2str(numFrames) ' total frames...']) 
disp('-     calculating pupil diameter...')
...
for i = 1:numFrames
    ...
    a = read(movieObj,i);
    a_crop = a(y:y+height,x:x+width,:);
    a_gs = rgb2gray(a_crop);
    
%     h=fspecial('gaussian', 3, 1);
%     a3=imfilter(a_crop,h,'replicate');
    a3 = medfilt2(a_gs,[5,5]);
    
    BW = im2bw(a3,thr/255);
    BWi = (BW-1).*-1;
    BW2 = bwareaopen(BWi, 20);
    
    if sum(sum(BW2)) > 0
        [pupil(i),outPts(:,:,i)] = maxDiameter(BW2);
    else
        % Can't handle first frame being blank...
        if i == 1
            pupil(i) = 0;
        else
            pupil(i) = pupil(i-1);
    ...
    img_corr = corr2(mean_frame,a_gs);
    
    blink(i) = img_corr;
    
    if play_video == 1
        blink_inds = find(blink<0.4);
        if i == 1
            hf = figure;
        end
        if rem(i,10) == 0 % refresh every 10th frame
        subplot(2,2,1)
            imagesc(a3), axis square, colormap('gray'), %caxis([0 100])
        subplot(2,2,2);
            imagesc(BW2), axis square            
        subplot(2,1,2);
            plot(pupil), hold on
            scatter(blink_inds,pupil(blink_inds),'r*')
            xlim([0,numFrames])
            ylim([1 max(pupil(1:i))+20])
            hold off
...
outData.movieFileName = file_name;
outData.pupil = pupil;
outData.outPts = outPts;
outData.mean_frame = mean_frame; 
outData.blink = blink;
...
if save_file == 1
    disp('-     saving results!')
    save([output_path file_name '.mat'],'outData');
else
...
%% Other Functions
function [outDiameter,maxPts] = maxDiameter(bwFrame)

%%%%%
%
%   Fast detection of object diameter
%       -Computes max diameter projection
%       -Image must be binarized black/white image
%
%   Usage:
%       [outDiameter,maxPts] = maxDiameter(BW_Img) 
%
%   Copyright MIT (c) 2017 Arthur Morrissette
%
%%%%%

edgeBoundaries = bwboundaries(bwFrame);

for i = 1:length(edgeBoundaries)
    if i == 1
        edgePts = edgeBoundaries{1};
    else
        edgePts = [edgePts;edgeBoundaries{i}];
    end
end

C = edgePts(:,1);
D = edgePts(:,2);
totalPts = sqrt(abs(C'-C).^2+abs(D'-D).^2);
distMax = max(max(totalPts));
% find maximum value in all of the distances
[E,F] = find(totalPts==distMax);

% write to output
maxPts = [edgePts(E(1),:);edgePts(F(1),:)];
outDiameter = distMax;

end

function [level,bw] = thresh_tool_rm(im,cmap,defaultLevel) %mainfunction
...
axes(h_ax2)
hold on
subimage(bw), axis off square ij
hold off
title('Segmented')
...
function ResetOriginalLevel(~,varargin) %subfunction
h_lev = getappdata(gcf,'h_lev');
...
%% Junk
%     if i > 6
%         mean_prev_pupil = mean(pupil(i-5:i-1));
%         std_prev_pupil = std(pupil(i-5:i-1));
%         std_prev_range = [mean_prev_pupil-3*std_prev_pupil,mean_prev_pupil+3*std_prev_pupil];
%         if pupil(i) < std_prev_range(1) || pupil(i) > std_prev_range(2)
%             disp(['Trial ' num2str(i) ' is out of range - reselect pupil?'])
%             [thr,~] = thresh_tool_rm(a_gs,'gray',thr);
%             
%             BW = im2bw(a3,thr/255);
%             BWi = (BW-1).*-1;
%             BW2 = bwareaopen(BWi, 400);
%     
%             if i ==1 
%                 baseROI = BW2;
%             end
