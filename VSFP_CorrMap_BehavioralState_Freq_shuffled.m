...
%   Takes 100 random pairs of volt,hemo and pupil data to get the shuffeled
%   control data
%
...
function VSFP_CorrMap_BehavioralState_Freq_shuffled(min_sec, filterType)
    ...
    FolderName = [startFile,'\VSFP ButterFly\Data\VSFP_CorrMap_BehavioralState_Freq_shuffled\', filterType,'_',num2str(min_sec),'\']; 
    ...
    k = 1;
    while k <= 100 
            % randomly pick two trial numbers from the list of all good trials
            rand_trial = randsample(length(all_trials_shuffle),2);
            ...
            % load the imaging data for this trial
            ...
            if length(pupil_data2x) > length(data_volt)
                pupil_data2x = pupil_data2x(1:length(data_volt));
                face_data2x = face_data2x(1:length(data_volt));
            else 
                data_volt = data_volt(:,:,1:length(pupil_data2x));
                data_hemo = data_hemo(:,:,1:length(pupil_data2x));
            end
            ...
            if move_epoch_num ~= 0
                ...
                % save figures
                %FolderName = 'X:\keilholz-lab\Lisa\VSFP ButterFly\Data\VSFP_CorrMap_Face_Freq_Avg\';   % Your destination folder
