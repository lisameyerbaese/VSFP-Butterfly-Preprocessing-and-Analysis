...
%   1) take Orofacial Mvmt and obtains correlation map for volt and hemo
%      (low pass <5Hz) signals
%
% Inputs: 
%        optional mice number, or list of numbers i.e.
%        VSFP_CorrMap_Face('VSFP24','VSFP25') or VSFP_CorrMap_Face('VSFP24')
...
function VSFP_CorrMap_Face(varargin)
    ...
    FolderName = [startFile, '\VSFP ButterFly\Data\VSFP_CorrMap_Face\'];   % Your destination folder
    ...
    for m = 1: length(all_mice) 
        ...
        for k = 1:length(all_trials)
            %get corresponding imaging data
            ...
            % Correlation With Orofacial Mvmt At Baseline
            corr_map_temp = fastCorr2(data_temp_2d(:,1:length(face_data2x))',face_data2x');
            ...
            hold off
            alpha(a,.2)
            title('Max Corr Coef Hemo and Orofacial')
            ...
            hold off
            alpha(a,.2)
            title('Max Corr Coef Volt and Orofacial')
            axis off;
        ...
        hold off
        alpha(a,.2)
        title('Max Corr Coef Hemo and Orofacial Mvmt')
        ...
        hold off
        alpha(a,.2)
        title('Max Corr Coef Volt and Orofacial Mvmt')
        axis off;
